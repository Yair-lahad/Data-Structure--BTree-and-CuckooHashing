...
public class BTree<T extends Comparable<T>> {
    ...
    //Task 2.1
    public boolean insert(T value) {   // as shown in class
        if (root == null) {
    ...
    private boolean insert(Node<T> oldroot ,T value) {
		// an algorithm as shown is class.
            if (oldroot.numberOfKeys()== maxKeySize) {
                Node<T> newroot = new Node<T>(null,maxKeySize,maxChildrenSize);// crate new empty node
                root = newroot;// making the root
                newroot.childrenSize= 0;// isn't leaf
                newroot.keysSize=0;//now is empty
                newroot.addChild(oldroot); //the root is a son of the new root
                split(oldroot);// split child - oldroot is the child of newroot therefore split oldroot
                return insertNonfull(newroot,value);
            } else
                return insertNonfull(oldroot,value);
    }
    
    private boolean insertNonfull(Node<T> target, T value) {
        int i = 0;
        if (target.numberOfChildren()==0) { // target is a leaf
            target.addKey(value); // should update automatically in the keysSize field;
            size++;
            return true;
        } else {
			// move forward the pointer
            while (i<target.keysSize && value.compareTo(target.getKey(i))>=0) {
            	i++;
            }
            if (target.getChild(i).numberOfKeys()== maxKeySize){ // if the child has maximum keys split it
                this.split(target.getChild(i));
                while (i< target.childrenSize-1 && value.compareTo(target.getKey(i))>0)
                    i++;
            }
            return insertNonfull(target.getChild(i), value);
        } 
    }
	
    public T delete(T value) {
        T output;
        // calling a private method which deletes and returns key value
        output=delete(value,root);
        size--;
        return output;
    }
    private T delete(T keyToDel, Node<T> nodeToDel) {
        if (nodeToDel == null) return null;
        T output = keyToDel;
        int ind= nodeToDel.indexOf(keyToDel); // indicates if key in this node
        // if element to remove is a leaf
        if (ind != -1 && nodeToDel.numberOfChildren() == 0) {
            output = nodeToDel.removeKey(keyToDel);
        }
        // if element to remove is not a leaf
        else if (ind != -1) {
            Node<T> left = nodeToDel.getChild(ind);
            Node<T> right = nodeToDel.getChild(ind+1);
            // removing by order- if left son has enough keys, replace with predecessor key and delete original keyToDel
            if (left.numberOfKeys() > minKeySize) {
                T replace = left.getMax(); // private method which returns predecessor of key
                output = nodeToDel.removeKey(keyToDel); // enough keys in order to delete
                nodeToDel.addKey(replace);  // inserting the pred key into current node
                delete(replace, left); // delete the pred key from its original leaf
            }
            // if left son does not have enough keys, replace with right successor and delete original keyToDel
            else if (right.numberOfKeys() > minKeySize) {
                T replace = right.getMin(); // private method which returns predecessor of key
                output = nodeToDel.removeKey(keyToDel); // enough keys in order to delete
                nodeToDel.addKey(replace);  // inserting the succ key into current node
                delete(replace, right); // delete the succ key from its original leaf
            }
            // left and right sons does not have enough keys, merge all and delete original keyToDel
            else {
                if(ind==0)
                    combined(left);
                else
                    combined(right);
                Node<T> mergedNode = getNode(keyToDel);
                output = delete(keyToDel, mergedNode);
            }
        } // continue to next node
        else {
            Node<T> xci = nodeToDel.findChildWithKey(keyToDel); // next node
            if (xci.numberOfKeys() == minKeySize) // need fixing
                combined(xci);
            xci = nodeToDel.findChildWithKey(keyToDel);
            output = delete(keyToDel, xci); // continue the deletion
        }
        return output;
    }
    //Task 2.2
    public boolean insert2pass(T value) {
        ...
        if (root == null) {
            ...
            Node<T> current = root;
            Node<T> lastNonFull = root;
            // finding leaf's path to insert our value
            while (current.numberOfChildren() != 0) {
                int i = 0;
                if (current.keysSize < maxKeySize)
                    lastNonFull = current; // set node to fix from ( in the second pass)
                while (i < current.keysSize && value.compareTo(current.getKey(i)) > 0) {
                    i++;
                }
                current = current.getChild(i); // advancing to correct child
            }
            // if leaf has room to insert (less than 2t-1 keys)
            if(current.numberOfKeys()<maxKeySize){
                current.addKey(value);
                size++;
                return true;
            }
            // if leaf is full, fix from the lowest non full node until you reach the leaf, fix the leaf and insert value
            return insert(lastNonFull,value);
        }
    }
    ...
    private boolean combined(Node<T> node) {
        ...
        if (leftNeighbor != null && leftNeighborSize > minKeySize) {
            ...
            if (rightNeighbor != null && rightNeighborSize > minKeySize) { // if can shift from right
    ...
    private static class Node<T extends Comparable<T>> {
        ...
        private T getMax(){
            Node <T> mostRight=this;
            while(mostRight.numberOfChildren() > 0)
                mostRight = mostRight.getChild(mostRight.numberOfChildren()-1);

            return mostRight.getKey(mostRight.numberOfKeys()-1);
        }

        private T getMin(){
            Node <T> mostLeft=this;
            while(mostLeft.numberOfChildren() > 0)
                mostLeft = mostLeft.getChild(0);

            return mostLeft.getKey(0);
        }

        public Node<T> findChildWithKey(T keyToDel) {
            int i = 0;
            while(i < numberOfKeys() && keyToDel.compareTo(getKey(i)) > 0){
                i++;
            }
